...
fit
```
The accuracy is 99.0% so we expect an out of sample error of 1% or more.
### Cross validation
...
cm$overall[1]
```
The out of sample error is then 0.6%. It is less than the expected 1%, which seems (too) good to be true. An explanation is that the data used to fit the model are obeservations from 5 individuals. Our training set is based on 80% of the data collected; inside that training set the random forests draws crossvalidation data to fit the model. By luck the tests data fits better than the crossvalidation sets extracted from the training set by the random forest algorithm.
